/* Kuro — shared cat drawing (browser + headless).
   Draws a front-3/4 sitting pixel cat that kneads two keyboard keys when typing. */
(function (root) {
  'use strict';
  ...
  // ---- grid raster helpers ----
  function makeGrid() { return new Int8Array(GW * GH); }
  function set(g, x, y, role) { if (x >= 0 && x < GW && y >= 0 && y < GH) g[y * GW + x] = role; }
  function get(g, x, y) { return (x < 0 || x >= GW || y < 0 || y >= GH) ? -1 : g[y * GW + x]; }
  function disc(g, cx, cy, r, role) {
    const r2 = r * r;
  ...
  function ellipse(g, cx, cy, rx, ry, role) {
  ...
  function rect(g, x0, y0, w, h, role) {
    for (let y = Math.round(y0); y < Math.round(y0 + h); y++)
      for (let x = Math.round(x0); x < Math.round(x0 + w); x++) set(g, x, y, role);
  }
  function tri(g, ax, ay, bx, by, cx, cy, role) {
  ...
  // ---- paint grid → canvas ----
  function palette(settings, heat) {
  ...
  // main entry
  function draw(ctx, view, state, settings) {
  ...
  const API = { draw, GW, GH };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
  if (root) root.CatDraw = API;
